...
{
	//不带参数的lamda表达式
	auto basicaLamda = [] {std::cout << "hello world" << std::endl; };
	basicaLamda();
	//带参数的lamda表达式
	auto add = [](int a, int b) -> int {return a + b; };
	auto multiplay = [](int a, int b) {return a * b; };
	int sum = add(2, 5);
	std::cout << sum << std::endl;
	int product = multiplay(2, 5);
...
void test_lambda_3()
{
	...
	sort(myvec.begin(), myvec.end(), std::less<int>()); // 旧式做法
	std::cout << "predicate function:" << endl;
	for (int it : myvec)
		std::cout << it << ' ';
	std::cout << std::endl;
